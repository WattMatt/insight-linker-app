...
export function createProgressBar(
  ...
  options?: {
    ...
    height?: number;
    showLabel?: boolean;
    labelPosition?: 'right' | 'inside' | 'below';
  }
): Content {
  const { width = 200, height = 12, showLabel = true, labelPosition = 'right' } = options || {};
  
  const fillWidth = Math.max(0, Math.min(100, percentage)) / 100 * width;
  const fillColor = percentage >= 80 ? COLORS.success : percentage >= 50 ? COLORS.warning : COLORS.error;
  ...
  if (labelPosition === 'right') {
    return {
      columns: [
        ...
        { text: `${Math.round(percentage)}%`, fontSize: 9, bold: true, margin: [5, 1, 0, 0], width: 35 },
      ],
    };
  }
  ...
  if (labelPosition === 'below') {
    return {
      stack: [
        barContent,
        { text: `${Math.round(percentage)}%`, fontSize: 8, alignment: 'center', margin: [0, 2, 0, 0] },
      ],
    };
  }
  
  // Inside - label overlaid on bar
  return {
    stack: [
      barContent,
      { text: `${Math.round(percentage)}%`, fontSize: 8, bold: true, color: COLORS.white, relativePosition: { x: width / 2 - 10, y: -height + 2 } },
    ],
  };
}
...
/**
 * Create a signature section for documents
 */
export function createSignatureSection(
  signatures: SignatureData[],
  title: string = 'Signatures'
): Content[] {
  ...
  const signatureRows = signatures.map(sig => {
    const signedDate = sig.signedAt instanceof Date 
      ? sig.signedAt.toLocaleDateString('en-GB') 
      : new Date(sig.signedAt).toLocaleDateString('en-GB');
    
    const row: Content[] = [
      // Signature image or placeholder
      sig.signatureDataUrl 
        ? { image: sig.signatureDataUrl, width: 100, height: 40 }
        : { text: '[Signature on file]', fontSize: 9, italics: true, color: COLORS.textMuted },
      // Signer info
      {
        stack: [
          { text: sig.signerName, fontSize: 10, bold: true },
          { text: sig.signerType, fontSize: 9, color: COLORS.textSecondary },
          sig.signerEmail ? { text: sig.signerEmail, fontSize: 8, color: COLORS.textMuted } : { text: '' },
          { text: `Signed: ${signedDate}`, fontSize: 8, color: COLORS.textMuted },
        ],
      },
    ];
