...
- ⏳ **Step 2 REMAINING (Arno, in the Lovable project)** — deploy the redirect shim so `wm-compliance.lovable.app` codes re-route. Must redirect to **insight-linker-app.vercel.app** (NOT watsonmattheus.com — avoids a loop while DNS cache clears). Snippet below.
