...
- **2026-06-21 — Site Documents management layer shipped (origin/main `65f71ad`).** The Site Detail → Documents tab gained Admin-only document management: rename / move (single + bulk) / delete documents, category rename/reorder/empty/delete, per-document audit history, file metadata, and multi-file upload + validation. New lib `src/lib/documents/` + `MoveDocumentsDialog`/`DocumentHistoryDialog`. Migration `20260621120000` (site_documents metadata cols + `is_system` on both category tables). **Docs updated:** `APPLICATION_SPEC.md`, `07-components-hooks-lib/components-site.md`, `02-data-model/tables-04.md` (site_documents + site_document_categories), `02-data-model/tables-02.md` (document_categories), `01-architecture.md`, `04-routes/admin-sites-inspections.md`.
