...
serve(async (req)=>{
  ...
  try {
    ...
    let query = supabase.from('subsections').select(`
        id,
        name,
        coc_type,
    ...
    if (subsectionsError) {
      ...
      return new Response(JSON.stringify({
        ...
        details: subsectionsError.message
      }), {
        status: 500,
        headers: {
          ...corsHeaders,
    ...
    const { data: existingValidations } = await supabase.from('coc_validations').select('document_id').in('document_id', documentIds);
