...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { imageUrl, bucket, fileName } = await req.json();
    ...
    const { data: uploadData, error: uploadError } = await supabase.storage.from(bucket).upload(cleanFileName, imageBuffer, {
      contentType,
      upsert: false
    });
    if (uploadError) {
