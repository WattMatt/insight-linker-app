...
serve(async (req)=>{
  ...
  try {
    ...
    if (!fileResponse.ok) {
      ...
      const errorDetail = fileResponse.status === 404 ? 'File not found - the Firebase Storage URL may have expired or the file no longer exists' : `Firebase Storage returned ${fileResponse.status}: ${fileResponse.statusText}`;
    ...
    const { data: existingFile } = await supabase.storage.from(targetBucket).list(filename.split('/').slice(0, -1).join('/'), {
    ...
    const { data, error } = await supabase.storage.from(targetBucket).upload(filename, fileBlob, {
      contentType,
      upsert: false
    });
    if (error) {
