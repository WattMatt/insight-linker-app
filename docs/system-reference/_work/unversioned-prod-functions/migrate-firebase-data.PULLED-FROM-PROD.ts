...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { data: existingTable, error: checkError } = await supabase.from(tableName).select('*').limit(1);
    ...
    // Insert data
    const { data: insertedData, error: insertError } = await supabase.from(targetTable).insert(records).select();
    if (insertError) {
