...
- **Progress (2026-06-11):**
  - 🟢 7 dead anon-reachable fns DELETED from prod (generate-pdf-google/-browserless/-pdfmake/-docx, fix-inspection-photos, fix-tenant-images, detect-schematic-regions — 0 callers, verified 404; source kept in repo).
  - 🟢 **7 live fns HARDENED + DEPLOYED 2026-06-11** (commit `13b1991`): in-handler `getUser` guard requires an authenticated user → `generate-pdf`, `generate-inspection-pdf`, `send-email`, `compress-image`, `batch-compress-images`, `offline-review`, `verify-fix`. Verified: anon-key probe → 401 (5/7) or 400-then-401 (2/7 validate body before the guard — no anon side effect reachable either way; guard sits before every privileged write/upload/LLM/email call). send-email/offline-review/verify-fix had no supabase client → a minimal service-role client was added solely for getUser.
  - 🟢 **`extract-coc` SSRF CLOSED 2026-06-11** (commit deployed): the direct-fetch fallback now rejects any URL not on the project's own Supabase host. (Was already auth-gated + prefers the verified `documentRow.file_url`; this closes the residual authenticated-Admin arbitrary-URL fetch.)
  - **Remaining need a DECISION, not a quick fix (investigated 2026-06-11):**
    - `api-reports` "IDOR" is **by design** — `api_clients` has NO tenant-scope column (`name`/`client_id`/`scopes`/`created_by` only); an API client is a global, admin-created credential. Scoping it to a tenant = a product decision + schema change, not a bug fix. **Decision needed: should API clients be tenant-scoped?**
    - `oauth-token` stores `client_secret` in **plaintext**; the real fix is hashing (migration + change to client-creation + **risks breaking live API clients**). The cheap constant-time tweak is low-value. **Decision/plan needed before touching a live OAuth endpoint.**
    - `save-template` + `template-sync` — fail-open token; fix depends on **G-SEC-18** (DocBuilder: delete vs harden).
    - `qr-redirect` + `log-auth-event` — intentionally public; low risk (leave or rate-limit).
  - Minor follow-up: in the 2 hardened fns returning 400 first, move the auth guard ABOVE input validation (auth-first ordering) — cosmetic, security already holds.
...
### G-SEC-16 · COC validation can be gamed / is inconsistent ✅ DISSOLVED 2026-06-12
- **Resolution:** the entire COC auto-validation engine was removed (2026-06-12). `validate-coc`/`extract-coc` edge functions deleted from prod; the `coc_validations`/`coc_extractions`/`coc_validation_settings`/`coc_local_validations`/`coc_compliance_photos` tables dropped; the `sync_coc_compliance_status` trigger dropped. COC is now a **manual** per-subsection verdict (`coc_status`), and `is_compliant` has a **single owner** — the inspection recompute (`apply_subsection_recompute`) with a COC gate (a failed/expired required COC forces non-compliant). All three sub-issues are moot: no `testSettings`, no second writer, no optimistic pre-write. See `docs/superpowers/COC-VALIDATION-STRIPOUT-TRACKER.md` + plan `docs/superpowers/plans/2026-06-11-coc-manual-workflow.md`. (Original gap retained below for history.)
