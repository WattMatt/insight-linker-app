...
- **Progress (2026-06-11):**
  ...
  - STILL OPEN: `save-template` + `template-sync` (fail-open token — fix depends on **G-SEC-18** DocBuilder decision: delete vs harden); `oauth-token` (plaintext secret + non-constant-time compare → constant-time fix); `api-reports` (IDOR — needs tenant scoping); `extract-coc` (already auth'd; SSRF via arbitrary documentUrl → bind to documentId); `qr-redirect` + `log-auth-event` (intentionally public; low risk — leave or rate-limit).
  - Minor follow-up: in the 2 functions returning 400 first, move the auth guard ABOVE input validation (auth-first ordering) — cosmetic, security already holds.
