...
interface CodeReviewRequest {
  ...
  reviewType: 'full' | 'security' | 'performance' | 'architecture' | 'sans-compliance';
  focusAreas?: string[];
}
...
serve(async (req) => {
  ...
  try {
    const { codeFiles, reviewType = 'full', focusAreas = [] }: CodeReviewRequest = await req.json();
...
This development prompt should be self-contained and actionable without needing to reference the original review.`;
    ...
    if (!response.ok) {
      ...
      if (response.status === 401) {
        return new Response(
          JSON.stringify({ error: "Invalid Abacus AI API key. Please check your configuration." }),
