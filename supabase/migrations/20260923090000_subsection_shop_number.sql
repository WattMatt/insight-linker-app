-- Asset Verification: shop numbers on subsections (2026-09-23).
--
-- At line-shop sites the subsection IS the shop, and the Asset Verification tab ties each
-- register row to its shop by shop number. Until now the only place a shop number lived was
-- the subsection NAME ("SHOP G01-G06"), which fails for shops named after their tenant
-- ("Shop 1 - Shoprite" style) and for boards ("Main DB F1 1st Floor North" = register DB-F1).
-- shop_number is optional; when blank the app reads the number from the name as before.
--
-- Additive only: one nullable column, and the public review RPC returns it. The function body
-- below is pg_get_functiondef() of the live function with ONE change — 'shop_number' added to
-- the subsections object — never a hand restatement.

ALTER TABLE public.subsections ADD COLUMN IF NOT EXISTS shop_number text;

COMMENT ON COLUMN public.subsections.shop_number IS
  'Shop / premises number this subsection stands for (e.g. G01-G06, DB-F1). Links it to the asset register. Blank = read from the name.';
  ...
  RETURN jsonb_build_object(
    ...
    'subsections', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        ...
        'metering_status', sub.metering_status, 'meter_serial_number', sub.meter_serial_number, 'shop_number', sub.shop_number
      ) ORDER BY sub.name)
